(function () {
  const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  const FLOW = {
    abc: ["nA", "nB", "nC"],
    abcd: ["nA", "nB", "nC", "nD"],
    qwen: ["nA", "nB", "nC", "nQ"],
  };
  let tl;

  function flowEls() {
    return document.querySelectorAll("#flowWorld .station");
  }

  function drawLine(el, duration) {
    if (!el || typeof gsap === "undefined") return;
    const len = el.getTotalLength ? el.getTotalLength() : 400;
    gsap.fromTo(
      el,
      { strokeDasharray: len, strokeDashoffset: len },
      { strokeDashoffset: 0, duration: duration || 0.7, ease: "power2.inOut" }
    );
  }

  function renderTex(root) {
    if (typeof katex === "undefined") return;
    (root || document).querySelectorAll(".tex").forEach((el) => {
      const src = el.getAttribute("data-tex");
      if (!src) return;
      try {
        katex.render(src, el, { throwOnError: false, displayMode: true });
      } catch (e) {
        el.textContent = src;
      }
    });
  }

  function clearNodeProps() {
    if (typeof gsap === "undefined") return;
    ["nD", "nQ"].forEach((id) => {
      const el = document.getElementById(id);
      if (el) {
        el.classList.remove("dead");
        gsap.set(el, { clearProps: "transform,opacity,x,y,rotate,scale" });
      }
    });
  }

  function setFlow(mode) {
    const ids = FLOW[mode] || [];
    flowEls().forEach((el) => el.classList.toggle("is-on", ids.includes(el.id)));
    const cap = document.getElementById("flowCap");
    if (cap) {
      cap.innerHTML =
        mode === "qwen"
          ? "<i>Fig. 1.</i> D is now Qwen3-32B, offline on Bedrock. A, B and C still run on this laptop. Conversion never ships inside Chrome."
          : mode === "abcd"
            ? "<i>Fig. 1.</i> Failed hypothesis: add transliteration as a fourth on-device step. It only changed the script."
            : "<i>Fig. 1.</i> Inference is three on-device steps. Conversion is training-time work and is not on this line.";
    }
  }

  function morphToQwen() {
    const d = document.getElementById("nD");
    const q = document.getElementById("nQ");
    if (!d || !q || typeof gsap === "undefined" || reduce) {
      setFlow("qwen");
      return;
    }
    clearNodeProps();
    setFlow("abcd");
    d.classList.add("dead");
    const local = gsap.timeline();
    local
      .to({}, { duration: 0.35 })
      .to(d, { x: 140, y: -10, rotate: 12, opacity: 0, duration: 0.55, ease: "power3.in" })
      .add(() => {
        d.classList.remove("dead");
        setFlow("qwen");
      })
      .fromTo(
        q,
        { x: -72, opacity: 0, scale: 0.92 },
        { x: 0, opacity: 1, scale: 1, duration: 0.6, ease: "power3.out" }
      )
      .add(() => gsap.set(q, { x: 0, y: 0, rotate: 0, opacity: 1, scale: 1 }));
  }

  function enterFlow(mode, morph) {
    document.body.classList.add("flowing");
    if (morph) {
      morphToQwen();
      return;
    }
    clearNodeProps();
    setFlow(mode);
    if (typeof gsap === "undefined" || reduce) return;
    const shown = Array.from(flowEls()).filter((el) => el.classList.contains("is-on"));
    gsap.fromTo(
      shown,
      { y: 18 },
      { y: 0, duration: 0.5, stagger: 0.16, ease: "power3.out" }
    );
  }

  function growBars(slide) {
    slide.querySelectorAll(".bar i").forEach((el, i) => {
      const w = Math.max(0, Math.min(1, parseFloat(el.dataset.w || "0")));
      if (typeof gsap === "undefined" || reduce) {
        el.style.width = w * 100 + "%";
        return;
      }
      gsap.fromTo(
        el,
        { width: "0%" },
        { width: w * 100 + "%", duration: 0.95, delay: 0.12 + i * 0.08, ease: "power3.out" }
      );
    });
  }

  window.playSlideMotion = function (slide, index) {
    if (!slide) return;
    if (tl) tl.kill();

    const flow = slide.dataset.flow || "";
    const thought = slide.dataset.thought === "1";
    const morph = slide.dataset.morph === "1";
    const x = Number(slide.dataset.token || 40);
    const pct = ((x - 40) / 1120) * 88;

    document.getElementById("thought")?.classList.toggle("on", thought);
    renderTex(slide);

    if (flow) enterFlow(flow, morph);
    else {
      document.body.classList.remove("flowing");
      clearNodeProps();
      setFlow("");
    }

    growBars(slide);

    const bits = slide.querySelectorAll("[data-in]");
    const tok = document.getElementById("token");
    const viz = slide.querySelector(".viz");

    if (typeof gsap === "undefined" || reduce) {
      if (tok) tok.style.left = pct + "%";
      return;
    }

    tl = gsap.timeline();
    if (bits.length) {
      tl.fromTo(
        bits,
        { y: 18 },
        { y: 0, duration: 0.48, stagger: 0.08, ease: "power3.out" }
      );
    }
    slide.querySelectorAll("[data-draw]").forEach((el) => drawLine(el, 0.9));
    if (viz && !flow) {
      tl.fromTo(viz, { y: 16 }, { y: 0, duration: 0.6, ease: "power3.out" }, 0);
    }
    if (thought) {
      const th = document.getElementById("thought");
      if (th) tl.fromTo(th, { x: 18 }, { x: 0, duration: 0.45, ease: "power3.out" }, 0.3);
    }
    if (tok) tl.to(tok, { left: pct + "%", duration: 0.75, ease: "power2.inOut" }, 0);
    if (window.SurakshaScene) window.SurakshaScene.enter(index);
  };

  document.addEventListener("DOMContentLoaded", () => renderTex(document));
})();
