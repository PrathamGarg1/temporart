...
// Particles disabled for this deck.
...
function updateMotif() {
  ...
  if (ch) ch.textContent = slide.dataset.chapter || '';
  const g = document.getElementById('goal');
  if (g && slide.dataset.goal) {
    g.innerHTML = slide.dataset.goal
      .replace(/^This slide/, "<b>This slide</b>")
      .replace(/^Goal/, "<b>Goal</b>");
  }
  const step = slide.dataset.step;
  const now = (slide.dataset.chapter || '').indexOf('This') === 0;
  document.querySelectorAll('#pipeMini i').forEach((el) => {
    el.classList.toggle('on', el.dataset.s === step);
    el.classList.toggle('now', el.dataset.s === step && now);
  });
}
...
function goTo(index) {
 ...
 if (location.hash !== '#' + (current+1)) history.replaceState(null, '', '#' + (current+1));
  ...
  if (typeof playSlideMotion === 'function') {
    requestAnimationFrame(() => playSlideMotion(slides[current], current));
  }
}
...
(function bootHash() {
  ...
  if (!Number.isNaN(n)) {
    current = -1;
    goTo(Math.max(0, Math.min(total - 1, n - 1)));
  } else if (typeof playSlideMotion === 'function') {
    playSlideMotion(slides[current], current);
  }
})();
